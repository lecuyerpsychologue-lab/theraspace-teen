...
const callAPI = async (mode, data={}, context={}, history=[]) => {
  try {
    ...
    if(mode==='GENERATE_IDENTITY_QS') return ["Ton surnom ?", "Ta passion ?", "Ton rêve ?"];
    if(mode==='ANALYZE_ECHO') return {weather:{icon:"Sun", text:"Beau fixe"}, analysis:"Tu gères bien."};
    if(mode==='CHAT_THERAPIST') return {text: "Je suis hors ligne."};
    return null;
  }
};
...
const Btn = ({ onClick, children, variant='primary', disabled, className="" }) => {
  const s = { primary: "bg-teal text-white shadow-lg", secondary: "bg-white border border-gray-200 text-gray-600", accent: "bg-coral text-white", gold: "bg-yellow text-white" };
...
const SOS = ({ close }) => (
  <div className="fixed inset-0 z-50 bg-white/95 flex items-center justify-center p-6 animate-pulse">
    <div className="w-full max-w-sm glass border-red-100 rounded-3xl p-8 text-center shadow-xl">
      <div className="w-24 h-24 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6 text-red-500 animate-bounce"><AlertTriangle size={48}/></div>
...
const Auth = ({ login }) => {
  ...
  return <div className="h-full flex flex-col items-center justify-center p-8 bg-bg">
    <div className="w-24 h-24 rounded-full bg-coral flex items-center justify-center shadow-lg mb-6"><div className="text-white font-black text-4xl">TS</div></div>
    <h1 className="text-4xl font-black text-text mb-2">TheraSpace</h1>
...
const Dashboard = ({ nav, user }) => {
  ...
  return <div className="h-full overflow-y-auto no-scroll p-6 pt-10 pb-32 space-y-6 relative">
    ...
    <header className="flex justify-between items-center mb-4">
        <div><h1 className="text-3xl font-black text-text">Salut {user.name}</h1><p className="text-teal text-sm font-medium">Espace Sécurisé</p></div>
    ...
    <div onClick={()=>nav('echo')} className="active:scale-95 transition-transform"><Glass className="p-8 flex justify-between items-center relative overflow-hidden h-40 bg-coral text-white"><div><h3 className="text-3xl font-black mb-2">ÉCHO</h3><p className="opacity-90 text-sm">Exploration guidée</p></div><Play size={40} fill="currentColor"/></Glass></div>
    <div className="grid grid-cols-2 gap-4">
        <Glass className="p-6 h-32 flex flex-col justify-between active:scale-95 bg-teal text-white" onClick={()=>nav('therapist')}><MessageCircle size={32}/><h4 className="font-bold text-lg">Psy Virtuel</h4></Glass>
        <Glass className="p-6 h-32 flex flex-col justify-between active:scale-95 bg-yellow text-white" onClick={()=>nav('oracle')}><Flame size={32}/><h4 className="font-bold text-lg">Oracle</h4></Glass>
    </div>
    <div className="grid grid-cols-2 gap-4">
...
const Echo = ({ exit }) => {
    ...
    if(step==='load') return <div className="h-full flex flex-col items-center justify-center text-center"><Sparkles className="animate-spin text-coral mb-4" size={48}/><h2 className="text-2xl font-bold text-gray-700">ÉCHO...</h2><p className="text-gray-400">Est-ce que ça résonne en toi ?</p></div>;
    if(step==='analyzing') return <div className="h-full flex flex-col items-center justify-center text-center"><Zap className="animate-pulse text-teal mb-4" size={48}/><p className="text-gray-600">Analyse de ta session...</p></div>;
    if(step==='feedback') return <div className="h-full p-6 pt-10 bg-white overflow-y-auto"><h2 className="text-2xl font-black mb-6 text-text">Bilan IA</h2><div className="bg-blue-50 p-6 rounded-2xl mb-4 text-center"><h3 className="font-bold text-blue-800">Météo</h3><p className="text-blue-600 text-sm mt-2">{aiRes?.weather?.text}</p></div><div className="bg-gray-50 p-6 rounded-2xl mb-6"><h3 className="font-bold text-gray-800 mb-2">Compréhension</h3><p className="text-gray-600 text-sm leading-relaxed">{aiRes?.analysis}</p></div><Btn onClick={exit} variant="secondary" className="w-full">Terminer</Btn></div>;

    const card = cards[idx] || {text:"..."};
    const interaction = curr.emotions.length > 0 || curr.strategies.length > 0;
    
    if(step==='play') return <div className="h-full flex flex-col p-6 pt-10 bg-coral text-white">
        ...
        <Glass className="flex-1 flex flex-col relative overflow-hidden bg-white/10 backdrop-blur-xl border-white/20">
            {!interaction ? 
             <div className="flex-1 flex flex-col justify-center items-center p-8 text-center anim-slide"><h2 className="text-2xl font-black leading-snug mb-12">"{card.text}"</h2><div className="grid grid-cols-2 gap-4 w-full mt-auto"><Btn className="bg-white/20 border border-white/40" onClick={()=>next()}>Pas vécu</Btn><Btn className="bg-white text-pink-500" onClick={()=>setCurr({...curr, emotions:['vu']})}>Vécu</Btn></div></div> : 
             <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-white rounded-3xl text-gray-800"><div className="space-y-6"><div><p className="text-gray-400 text-xs font-bold uppercase mb-2">Comment tu t'es senti(e) ?</p><div className="flex flex-wrap gap-2">{EMOTIONS.map(e=><button key={e.l} onClick={()=>setCurr({...curr, emotions:toggle(curr.emotions, e.l)})} className={`px-3 py-2 rounded-xl border text-sm font-bold flex gap-2 ${curr.emotions.includes(e.l)?'bg-pink-100 border-pink-200 text-pink-600':'bg-gray-50 border-gray-100 text-gray-500'}`}>{e.e} {e.l}</button>)}</div></div><div><p className="text-gray-400 text-xs font-bold uppercase mb-2">Ta réaction ?</p><div className="grid grid-cols-2 gap-2">{STRATEGIES.map(s=>{const I=s.i; return <button key={s.l} onClick={()=>setCurr({...curr, strategies:toggle(curr.strategies, s.l)})} className={`p-3 rounded-xl border text-sm font-bold flex flex-col items-center gap-2 ${curr.strategies.includes(s.l)?'bg-teal-100 border-teal-200 text-teal-700':'bg-gray-50 border-gray-100 text-gray-500'}`}><I size={18}/> {s.l}</button>})}</div></div></div><Btn onClick={next} className="mt-8 w-full">Suivant</Btn></div>
            }
        </Glass></div>;
};
...
const Oracle = ({ exit, ctx }) => {
    ...
    return <div className="h-full p-6 pt-10 flex flex-col bg-yellow text-text"><header className="flex justify-between items-center mb-8"><h2 className="text-2xl font-black flex gap-2"><Flame/> L'Oracle</h2><button onClick={exit}><X/></button></header>{!res ? <div className="flex-1 flex flex-col items-center justify-center text-center"><div onClick={ask} className="w-48 h-48 bg-white rounded-full flex items-center justify-center shadow-lg cursor-pointer active:scale-95 transition-transform">{load?<div className="animate-spin text-yellow">⌛</div>:<span className="font-black text-xl">RÉVÉLATION</span>}</div></div> : <div className="flex-1 bg-white rounded-[30px] p-8 shadow-xl flex flex-col justify-center text-gray-800"><span className="text-xs font-bold text-orange-400 uppercase tracking-widest mb-4 block">{res.title}</span><p className="text-xl leading-relaxed italic mb-6">"{res.story}"</p><div className="bg-orange-50 p-4 rounded-xl border border-orange-100 mb-4"><p className="text-orange-800 text-sm font-bold">Leçon : {res.moral}</p></div><div className="text-xs text-gray-400 text-right italic mb-6">Source : {res.source}</div><Btn onClick={ask} variant="secondary" className="w-full !border-gray-200 !text-gray-500">Nouveau message</Btn></div>}</div>;
};
...
const Journal = ({ exit }) => {
    ...
    return <div className="h-full bg-bg flex flex-col relative overflow-hidden">{!add ? <><header className="p-6 pt-10 flex justify-between items-center"><h2 className="text-2xl font-black flex gap-3 text-teal"><Book color="#2A9D8F"/> Note à moi-même</h2><button onClick={exit}><X color="#AAA"/></button></header><div className="flex-1 overflow-y-auto p-6 space-y-4 pb-32 no-scroll">{notes.map(n=><div key
